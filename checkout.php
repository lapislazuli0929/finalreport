		...
		<div class=>
            <?php
                ...
                if($result=mysqli_query($link,$SQL)){
                        ...
                        echo "<table border='1'>";
                        echo "<td>"."飲料"."</td/><td>"."甜度"."</td><td>"."冰塊"."</td/><td>"."價格"."</td/><td>"."意見"."</td/><td>"."編輯"."</td>";
                    while($row=mysqli_fetch_assoc($result)){
                      echo "<tr>";
                      
                      echo "<td>".$row["drink"]."</td><td>".$row["sweet"]."</td><td>".$row["ice"]."</td><td>".$row["price"]."</td><td>".$row["comment"]."</td><td><a href='delete.php?uNo=".$row["sNo"]."'>刪除</a></td>"."</br>";
                      echo "</tr>";

                      $sumprice+=$row["price"];
                      $drink = $row["drink"].$row["sweet"].$row["ice"].$row["comment"];
                      $alldrink= $alldrink.$drink.'</br>';
                    }
                ...
                echo "總價:".$sumprice;
                    ...
                    $_SESSION["drink"]=$alldrink;

                $SQL2="SELECT * FROM logininfo";
                if($result2=mysqli_query($link,$SQL2)){
                    while($row2=mysqli_fetch_assoc($result2)){
                        echo "<a href='mailsend.php'?UNo=".$row2["Uno"]."></a>";
                    }
                }
